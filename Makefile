...
clean:
	$(RM) $(TMPDIR)/build/*.o $(TMPDIR)/build/*.hi src/libskb.so src/Skb_stub.h src/SkbFFI_stub.h src/KeySymbolDefinitions.hs src/libskb-xkbcommon.so $(WLD)/lib/libskb.so $(WLD)/lib/libskb-xkbcommon.so $(WLD)/include/Skb_stub.h $(WLD)/include/skb.h
...
src/libskb.so: src/**/*.hs src/skb.c src/skb.h
	...
	cd src && \
		TMPDIR=$(TMPDIR) stack exec ghc -- --make \
		    ...
		    -I$(WLD)/include \
		    -o libskb.so SkbFFI.hs Skb.hs skb.c \
...
src/libskb-xkbcommon.so: src/**/*.hs src/xkb.c src/skb.c src/skb.h
	...
	cd src && \
		TMPDIR=$(TMPDIR) stack exec ghc -- --make \
		    ...
		    -O2 -dynamic -shared -fPIC \
		    -I$(WLD)/include \
		    -o libskb-xkbcommon.so SkbFFI.hs Skb.hs skb.c xkb.c \
		    -l"HSrts-ghc$(GHCVERSION)"
...
ifndef WLD
	...
	cp src/SkbFFI_stub.h $(WLD)/include/
